...
| 페이퍼 설정 | [`config/paper_swing.yaml`](config/paper_swing.yaml) |
| **타임프레임** | **4h** |
| 심볼 (검증) | BTC/USDT |
| **운용 상태 (2026-04-19~)** | Oracle Cloud 춘천 VM 24/7 페이퍼 가동 (Binance Mainnet 시세, systemd `crypto-bot.service`) |
...
## 6. 사용자 점진 운용 로드맵 (swing_pullback v2 기준, 2026-04-19 업데이트)

자본 500만원 + Max 구독 + 학습 우선 성향에 맞춘 실제 진행 로드맵.

| 단계 | 기간 | 자본 | 상태 | 핵심 |
| --- | --- | --- | --- | --- |
| ① 백테스트·전략 선정 | 완료 | 0원 | ✅ | 6구간 3.5년 백테스트 → swing_pullback v2 확정 |
| ② Oracle Cloud 페이퍼 24/7 | 2026-04-19 ~ 04-26 (7일) | 가상 | **🟢 진행 중** | systemd 가동, 텔레그램 `@KKHCryptoBot` 알림, 일일 리포트 연속 7회 확인 |
| ③ Upbit 실전 소액 | 2026-04-26~ (1~2주) | 25만원 (자본의 5%) | 대기 | `max_position_pct: 0.10`, `stop_loss_pct: 0.05` 권장 (백테스트 0.08 유지도 OK, 자본 적어 단위 손실 작음) |
| ④ 검증·증액 판단 | 지속 | 25만 → 50만 → 100만 | 대기 | 1~2개월 무사고 후 단계적 2배씩 증액 |

### 페이퍼 단계 핵심 인식

- swing_pullback v2 는 **연 6회 빈도** 설계라 7일 페이퍼 기간 내 매매 체결 0회가 **정상**
- 따라서 페이퍼는 "전략 검증" 이 아니라 **인프라 검증** (API 연결, 알림, systemd 재시작, 일일 리포트) 으로 한정
- 전략 검증은 이미 백테스트 6구간 3.5년으로 완료 (§0-A 참고)

### 절대 원칙

- **7일 연속 일일 리포트 발송 확인** 전까진 실전 금지 (인프라 검증 최저선)
- **이상 동작 1회라도 발견 시 즉시 중단** → 원인 분석 → 테스트 추가 → 재개
- **증액은 검증 후에만**: 일정 맞추려고 억지로 증액 금지
- 손절·서킷브레이커 임계값 **임의 완화 금지**
- **매매 빈도가 적다고 조건 완화 금지**: §0-A 기각 이력의 D1/D2/D3 실험이 증거
